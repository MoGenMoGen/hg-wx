<template>
  <div class="home">
    ...
    <div class="info">
      <div>
        <span>真实姓名</span>
        <input v-model="info.realName" placeholder-class="gray" :disabled="info.status==2 || !ifEdit"/>
        <image :src="pen" mode="widthFix" v-show="info.status==1 && ifEdit"/>
      </div>
      <div>
        <span>手机号码</span>
        <input v-model="info.phone" type="number" placeholder-class="gray" :disabled="info.status==2 || !ifEdit"/>
        <image :src="pen" mode="widthFix" v-show="info.status==1 && ifEdit"/>
      </div>
      <div>
        <span>身份证号</span>
        <input v-model="info.idCard" type="idcard" placeholder-class="gray" :disabled="info.status==2 || !ifEdit"/>
        <image :src="pen" mode="widthFix" v-show="info.status==1 && ifEdit"/>
      </div>
      <div>
        <span>家庭住址</span>
        <input v-model="info.house" placeholder-class="gray" :disabled="info.status==2 || !ifEdit"/>
        <image :src="pen" mode="widthFix" v-show="info.status==1 && ifEdit"/>
      </div>
      <div>
        <span>政治面貌</span>
        <picker @change="changePolitic" :value="Index" range-key="label" :range="StatusList" :disabled="!ifEdit">
          <view class="picker">
            {{info.politic>-1 ? StatusList[Index].label : "请选择政治面貌"}}
          </view>
        </picker>
        <image :src="pen" mode="widthFix" v-show="info.status==1 && ifEdit"/>
      </div>
      ...
      <div>
        <span>属      性</span>
        <picker @change="changeType" :value="info.type" :range="typeList" :disabled="info.status==2 || !ifEdit">
          <view class="picker">
            {{typeList[info.type-1]}}
          </view>
        </picker>
        <image :src="pen" mode="widthFix" v-show="info.status==1 && ifEdit"/>
      </div>
      <div>
        ...
        <p v-show="!ifEdit">{{info.abilityNm?info.abilityNm:'请选择兴趣爱好'}}</p>
        <p @click="hobbyShow=true" v-show="ifEdit">{{info.abilityNm?info.abilityNm:'请选择兴趣爱好'}}</p>
        <image :src="pen" mode="widthFix" v-show="ifEdit"/>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      save(){
        this.api.infoChange(this.info).then(()=>{
          wx.showToast({
            title: '保存成功',
            icon: 'success',
            duration: 2000
      ...
      changePolitic(e) {
        ...
        this.info.politic = this.StatusList[this.Index].value;
      },
      // 选择属性
      changeType(e){
        this.info.type = Number(e.mp.detail.value)+1
      },
      //选择兴趣爱好
      changeHobby(info){
        this.hobbyList = info
