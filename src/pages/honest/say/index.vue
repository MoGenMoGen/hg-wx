<template>
  <div class="home">
    ...
    <div class="info-time"><span>{{info.handleTime}}</span></div>
    <img :src="info.img" class="info-topImg" mode="aspectFill">
    <div class="info-box">
      <div class="info-item">
        <div class="info-item-title"><div></div><span>群众点题（说）</span></div>
        <div class="info-item-cont">{{info.theme}}</div>
      </div>
      <div class="info-item" style="margin-bottom: 30rpx;" v-if="info.status!=2">
        <div class="info-item-title"><div></div><span>当前状态</span></div>
        <div class="info-item-cont" v-if="info.status==1">审核中</div>
        <div class="info-item-cont" v-if="info.status==3">审核拒绝</div>
      </div>
      <div class="info-item" v-if="info.status==2">
        <div class="info-item-title"><div></div><span>村委会答复（办）</span></div>
        <div class="info-item-cont" v-html="info.eventHandle"></div>
      </div>
      <div class="info-item" v-if="info.status==2">
        <div class="info-item-title" style="justify-content: space-between;">
          <span style="display: flex;align-items: center;"><div></div><span>群众评价（评）</span></span>
      ...
      <div class="btn-class" @click="evaluate" v-if="info.isEvaluate!=1&&info.status==2">我参与</div>
      <div class="info-item" v-if="info.status==2">
        <div class="info-item-title" style="justify-content: space-between;">
          <span style="display: flex;align-items: center;"><div></div><span>村民留言（议）</span></span>
          <span class="info-item-message" @click="commentSubmit(1,'')">留言</span>
        </div>
        <div class="info-item-comment" v-for="(item,index) in list" :key="index">
          <div class="comment-top">
            ...
            <div class="comment-time">
              ...
              <!-- <span><img src="/static/images/msg-logo.png" mode="heightFix">120</span> -->
              </span>
              <span style="color: #599BFF;" @click="commentSubmit(2,item.userName)">回复</span>
            </div>
          </div>
    ...
    <div class="common-btn" @click="toPage('/pages/honest/apply/main?type=1')">我发现</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getDetail(data) {
        this.api.getSayDetail(data).then(res => {
      ...
      submit() {
        ...
        if(this.name) {
          data.cont = '@'+this.name+' '+this.commentCont
        }
